...
function TeacherCard(props: Props) {
    ...
    const onEditClick = () => {
        const path = `${teachersPath}/${props.id}`;
        history.push(path);
    };

    const deleteTitle = String.Format(translations.deleteX, translations.teachers);
    const deleteContent = String.Format(translations.deleteItemsConfirm, "1");
    const fullname = `${props.firstName} ${props.firstLastName}`;
    return <Card className={classes.card}>
        <CardContent>
            ...
            <Typography color="textSecondary" gutterBottom>
                {`${translations.identifierNumber}: ${props.identifierNumber}`}
            </Typography>
            <Typography color="textSecondary">
                {`${translations.priority}: ${props.priority}`}
            </Typography>
        </CardContent>
